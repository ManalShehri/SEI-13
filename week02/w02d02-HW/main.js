let greatestSumMulti = function(){
    ...
    let result = "";
   for (key in matrix){
        for(let i = 0 ; i<matrix.numbers1.length;i++){
            let sum = 0;
            for(let j = 0 ; j<matrix.numbers1.length;j++){
  ...
  result = "Array : "+x+ " has greatest sum : "+maxsum ;
  return result;
...
function ispangrams(str) {
 ...
 let text= panagram.join();
 console.log(ispangrams(text));
 ...
 const products = [
  ...
  let result1 = products.filter(priceLest).sort(sortName);
  
  console.log(result1);

function priceLest(item) {
  return item.price < 10;
}

function sortName(firstEl, secondEl) {
    const first = firstEl.name.toLowerCase();

    const second = secondEl.name.toLowerCase();
