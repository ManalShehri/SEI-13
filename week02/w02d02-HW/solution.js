...
let greatestSumMulti = function () {
    ...
    console.log(`The matrix with the largest sum is ${largest_sum} and has the sum of ${sum}`)
}
...
function checkPanagram(array){
    const alphabet = "abcdefghijklmnopqrstuvwxyz";
    var lettersFound = ""
    for(var i=0; i<array.length; i++){
        var wordLetters = array[i].split('');
        for(var j=0; j<wordLetters.length; j++){
            if(lettersFound.split('').includes(wordLetters[j].toLowerCase()) == false){
                lettersFound += wordLetters[j].toLowerCase()
            }
        }
    }
    if(lettersFound.split('').sort().join('') == alphabet){
        console.log('Array is panagram')
    }else{
        console.log('Array is not panagram')
    }
}
