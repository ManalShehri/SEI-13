...
In our examle exercise above we have only two butotns and therefore we added event listeners manually to both of them.
But what if we had 100 buttons and our task ws to make it so taht an alert pops up when the user clicks on any of the 100 buttons.
...
Consortium)](./imgs/event-propagation-flow.png)
...
```js
const parentElement = document.querySelector(".js-parent-element");
...
The code above first gets the <code>.js-parent-element</code> element from the
...
**Let's refactor the exercise to use event propagation.**
