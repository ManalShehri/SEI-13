...
function checkPrime(num) {
	for (var i = 2; i < num; i++) {
		if (num % i === 0) {
			return false; // that means  the number is prime, so return false to terminate the loop
		}
	}

	// if the for loop never returns false, it's not evenly divisible by any number, so it must be prime
	return true;
}
